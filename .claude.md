...
- **Language**: Rust 2024 edition, MSRV 1.93
- **TUI**: ratatui 0.28 + crossterm 0.28
- **Database**: tokio-postgres (PostgreSQL only, `Database` trait for testability)
- **Async**: tokio
- **CLI**: clap 4
- **Clipboard**: arboard
...
2. Main loop executes `Action` (e.g. `ExecuteQuery` spawns a tokio task)
3. Spawned tasks send results back via `mpsc` channel → `AppEvent::QueryCompleted`, `SchemaLoaded`, etc.
...
- **Database trait**: Abstract `Database` trait in `db/mod.rs` for testability. `PostgresProvider` is the sole implementation. Queries run via `tokio::spawn` + `mpsc` channel to avoid blocking the UI.
